...
const OUTPUT_FILE = path.join(BASE_DIR, 'EQTL_RS.csv');
...
function parseRows(html) {
  ...
  tbody.find('tr').each((_, tr) => {
    ...
    const email = emailEl.length ? emailEl.attr('title').trim() : $(tds[4]).text().trim();
    const td7 = $(tr).find('td.col7').first();
    const statusCol7 = td7.length ? td7.text().trim() : (tds.length > 6 ? $(tds[6]).text().trim() : '');
...
async function run() {
  ...
  try {
    await loginBackoffice(page, URL_BASE, 'RS');
    await page.close();
